...
def test_genre_entity_hash() -> None:
    ...
    assert hash(genre1) != hash(genre2)  # Same name but different descriptions, should have different hashes
    assert hash(genre1) != hash(genre3)  # Different names, should have different hashes
    assert hash(genre1) == hash(genre4)  # Same name and description, should have same hashes
